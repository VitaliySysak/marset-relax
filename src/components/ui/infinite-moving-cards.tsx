        ...
        {items.map((item, idx) => (
          ...
          >
            <blockquote className='flex flex-col justify-between cursor-default h-full'>
              <span className="relative z-20 text-sm leading-[1.6] text-[18px] text-gray-100">
                {item.quote}
              </span>
              <div className="relative z-20 mt-6 flex flex-row items-center justify-between">
                <span className="flex flex-col gap-1">
                  <img
                    src={item.photoUrl}
                    className="text-sm leading-[1.6] w-[48px] h-[48px] font-normal text-neutral-500 dark:text-gray-400"
                    alt={item.name}
                  />
                ...
                <div className='flex'>{[...Array(5)].map((_, index) => <img className='w-[28px] h-[28px]' key={index} src="/icons/star.png" alt='star' />)}</div>
              </div>
            </blockquote>
          </li>
        ))}
